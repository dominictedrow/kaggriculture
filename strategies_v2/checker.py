"""CHECKER strategy -- interspersed checkerboard commodity mix.

strategy.md: "Frequent commodity switching... High accessibility and
flexibility at cost of density" -- CHECKER should win only if diversified
market exposure is worth the lost specialization. Same crew size and tile
count as SOLID (single NW quadrant, no livestock) so the two are a clean
single-variable comparison: SOLID's melon/wheat/carrot tiles form one
contiguous block each, CHECKER's are interleaved tile-by-tile via
(x + y) % 3, forcing every crop's hand to range across the whole quadrant
instead of working a tight cluster.
"""
...
CROP_MAX_YIELD_DAY = {"MELON": 10, "WHEAT": 4, "CARROT": 3}
...
_NW = QUADRANT_TILES["NW"]
MELON_POSITIONS = [(x, y) for (x, y) in _NW if (x + y) % 3 == 0]
WHEAT_POSITIONS = [(x, y) for (x, y) in _NW if (x + y) % 3 == 1]
CARROT_POSITIONS = [(x, y) for (x, y) in _NW if (x + y) % 3 == 2]

MELON_GROUPS = chunk_indices(len(MELON_POSITIONS), 2)  # farmer + 1 hand
...
def _demo():
    ...
    assert final >= 0
    print("checker self-check OK, final money:", final)
...
def _run_ab():
    ...
    for opp in ["random", "starter"]:
        ...
        print("checker", "vs", opp, results[opp])
    out_path = Path(__file__).resolve().parent / "results" / "checker.json"
